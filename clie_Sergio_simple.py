...
nuevas cosas (por orden de aparición):
    He añadido algunas cosas a lo que subio berni, lo he subido aparte por si acaso
...
def insert_word(word, tema, table, letter):
    if (not(stop)):
        ...
        print_state("Lo siento pero alguien ya dió el STOP, pulsa intro para continuar")
...
def callback_jugadores(mqttc, userdata, msg):
    ...
    #
        ...
        mqttc.subscribe(choques+"/partidas/"+str(1))
    ...
    #
        ...
        let=ord(mensaje[-1])
        letra.value = let
        print_state("EMPIEZA UNA NUEVA RONDA, PULSA INTRO PARA CONTINUAR", True)
        jugar.value = 1
    #
    elif (msg.payload == b'STOP'):
        global stop
        if (not(stop)): #este jugador no ha hecho stop
            stop = True
    ...
    #
        ...
        print("Partida en marcha, esperando a la siguiente ronda")
